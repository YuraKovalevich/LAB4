package bsu.rfe.java.group6.lab4.kovalevich.var6A;
...
// Отобразить график
    ...
    // В записи целой части значения функции в точке
    //используются только чѐтные цифры
    int evenn(double x) {
        int count = 0;
        int y = (int) x;
        int y_temp = y;
        ...
        int[] reverb = new int[even];

        for (int j = count - 1, i = 0; i < even; j--, i++) {
            if (array[j] == 0) {
                i--;
                continue;
            }
        ...
        for (int i = 0; i < reverb.length; i++) {
            result += String.valueOf(reverb[i]);
        }
        if (result == "") {
            return 0;
        }
        int test = parseInt(result);
        return test;
    }
...
// Шаг 2 - Организовать цикл по всем точкам графика
            ...
            el.setFrame(center.getX() - 5.5, center.getY() - 5.5, 11, 11);
            ...
            canvas.draw(el); // Начертить контур маркера
            canvas.draw(marker); // Залить внутреннюю область маркера
        }
    }
...
// Вести левый "скат" стрелки в точку с относительными координатами (5,20)
            arrow.lineTo(arrow.getCurrentPoint().getX() + 5,
                    arrow.getCurrentPoint().getY() + 5.5);
